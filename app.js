...
function updateText() {
    ...
    for (var j = 0; j < field.upgrades.length; j++) {
        ...
        document.getElementById("upgrade" + upgrade.name).innerHTML = (upgrade.canBuy() ? "" + upgrade.displayName.replace("%name%", activeField.machineName) + " - $" + upgrade.price : "MAXED");
...
function loadGame() {
    ...
    if (gameDataString) {
        ...
        mulch = gameData.mulch;
        currentPosition = gameData.currentPosition;
        unlockedFields = gameData.unlockedFields;
        growthBonus = gameData.growthBonus;
        
        // Load the active field
        activeField = getField(gameData.activeField);
        activeField.generateField();

        // Restore all fields data
        fields = gameData.fields.map(fieldData => {
            const area = new Area(fieldData.name, fieldData.multiplierBuff, fieldData.initialBuff, fieldData.baseColor, fieldData.grownColor, fieldData.machineColor, fieldData.unlockPrice, fieldData.message, fieldData.value, fieldData.machineName, fieldData.whyDoIDoThis);
            area.totalMowed = fieldData.totalMowed;
            area.machineSpeed = fieldData.machineSpeed;
            area.machineWidth = fieldData.machineWidth;
            area.machineHeight = fieldData.machineHeight;
            area.growthAmount = fieldData.growthAmount;
            area.upgrades = fieldData.upgrades; // Assuming upgrades is simplistic and doesn't need to be instantiated again
            return area;
        });

        // Update UI and restart the game loop
        updateText();
        updateMoney();
        tick(activeField);
